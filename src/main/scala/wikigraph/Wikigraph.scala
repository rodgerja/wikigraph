...
final class Wikigraph(client: Wikipedia):
   ...
   *           Hint: You will have to perform conversions between collection types
   *           because retrieving the links from an article returns a `Set`, but
   *           the method `WikiResult.traverse` requires a `Seq`. Also, the method
   *           `WikiResult.traverse` returns a `Seq`, but, ultimately, you want to
   *           return a `Set`. Remember that you can use `.toSeq` and `.toSet`.
   */
  def namedLinks(of: ArticleId): WikiResult[Set[String]] = {
    val idsOfLinks: WikiResult[Set[ArticleId]] = client.linksFrom(of)
    val idsOfLinksSeq: WikiResult[Seq[ArticleId]] = idsOfLinks.map(_.toSeq)
    val names: WikiResult[Seq[String]] = idsOfLinksSeq.flatMap { articleIds =>
      WikiResult.traverse(articleIds)(client.nameOfArticle(_))
    }
    names.map(_.toSet)
  }
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      q.dequeueOption match
        ...
        case Some(((depth, page), rest)) if depth > maxDepth =>
          WikiResult.successful(None)

        case Some(((depth, page), rest)) =>
          client.linksFrom(page).flatMap { linked =>
            if linked.contains(target) then
              WikiResult.successful(Some(depth))
            else
              val nextVisited = visited ++ linked
              val newItems = (linked -- visited).map(id => (depth + 1, id))
              val nextQueue = rest.enqueueAll(newItems)
              iter(nextVisited, nextQueue)
          } fallbackTo iter(visited, rest)
    end iter
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] = {
    ...
    val idOf = titles.map(title => title -> client.searchId(title)).toMap

    WikiResult.traverse(titlePairs) { (t1, t2) =>
      idOf(t1).zip(idOf(t2)).flatMap { (id1, id2) =>
        breadthFirstSearch(id1, id2, maxDepth).map(distance => (t1, t2, distance))
      }
    }
  }
end Wikigraph
